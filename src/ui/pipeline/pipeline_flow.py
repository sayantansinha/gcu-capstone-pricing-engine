...
def _flow_graphviz_dot(status_map: Dict[str, str]) -> str:
    ...
    Flow stops at 'Modeling' (no Visual Tools / Reporting).
    """
    label_map = {
    ...
    nodes = []
    for s in steps:
        fill = STATUS_CONFIG.get(status_map.get(s, "not_started")).get(
            "bg_color", "#E0E0E0"
        )
        label = label_map.get(s, s)
        nodes.append(
            f'"{s}" [shape=rect, style="filled,rounded", fillcolor="{fill}", label=<{label}>]'
        )

    # Build edges (only until Modeling)
    edges = []
